...
async fn cli_post_inner(state: AppState, form: CliForm) -> Result<Response, ServerError> {
        ...
        .await
    {
        ...
        Err(gapes_core::CoreError::NotFound) => Ok(render(CliErrorTemplate {
            reason: "unknown pairing code — re-check the code shown by your CLI",
        })),
        Err(gapes_core::CoreError::AlreadyExists) => Ok(render(CliErrorTemplate {
            reason: "this pairing has already been approved",
        })),
        Err(gapes_core::CoreError::InvalidInput(msg)) => Ok(render(CliErrorTemplate {
