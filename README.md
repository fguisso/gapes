...
```sh
git clone https://github.com/meistrari/gapes.git && cd gapes
cargo build --release
...
| `gapes whoami`                         | Current device + which credential store you're on.      |
| `gapes deploy <dir> [--uuid X]`        | Zip + upload. Omit `--uuid` to create, pass it to update. |
| `gapes ls`                             | Your pages.                                             |
...
Multi-arch image (`amd64` / `arm64`) built from [`docker/Dockerfile`](docker/Dockerfile). Only Caddy is exposed on the host (`:80`, `:443`); `gapes-server` stays on the internal bridge. State persists in `docker/data/` and `docker/pages/`.

### Option B — install script + systemd + Caddy

```sh
curl -fsSL https://raw.githubusercontent.com/meistrari/gapes/main/packaging/install.sh \
  | sudo bash
```

The script downloads the right binary for your arch, verifies its SHA-256, creates a `gapes` system user, drops `/etc/gapes/pages.toml`, and installs a hardened systemd unit. Pin a release with `sudo GAPES_VERSION=v0.1.0 bash install.sh`.
...
sudo $EDITOR /etc/gapes/pages.toml     # public_url, trusted_proxy
...
```sh
cargo build --release -p gapes-server
sudo install -m 755 target/release/gapes-server /usr/local/bin/
sudo install -m 644 packaging/systemd/gapes-server.service /etc/systemd/system/
sudo systemctl daemon-reload && sudo systemctl enable --now gapes-server
```
...
cargo build --release -p gapes-server           # release binary
```
...
mise run test           # tests
mise run dev            # cargo-watch run -p gapes-server
...
- TLS is the proxy's job — `gapes-server` listens HTTP only and refuses non-loopback in `--dev`.
